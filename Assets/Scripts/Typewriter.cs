...
public class Typewriter : MonoBehaviour
{
    ...
    [SerializeField] private bool useReverseTypewriter = false;
    ...
    private void StartTypewriter()
    {
        ...
        if (useReverseTypewriter)
        {
            if (text != null)
            {
                text.text = writer;
                StartCoroutine("ReverseTypeWriterText");
            }

            if (tmpProText != null)
            {
                tmpProText.text = writer;
                StartCoroutine("ReverseTypeWriterTMP");
            }
        }
        else
        {
    ...
    IEnumerator ReverseTypeWriterText()
    {
        yield return new WaitForSeconds(delayBeforeStart);

        for (int i = writer.Length; i >= 0; i--)
        {
            text.text = writer.Substring(0, i);
            yield return new WaitForSeconds(timeBtwChars);
        }
    }

    IEnumerator ReverseTypeWriterTMP()
    {
        yield return new WaitForSeconds(delayBeforeStart);

        for (int i = writer.Length; i >= 0; i--)
        {
            tmpProText.text = writer.Substring(0, i);
            yield return new WaitForSeconds(timeBtwChars);
        }
    }
}